...
function getCategories()
{
  ...
  $conn = db_connect();
  try {
    $postss = $conn->query("SELECT * FROM `posts`")->fetchAll(PDO::FETCH_ASSOC);
...
function renderTemplate($path, $arr, $ide)
{
  $output = '';
  if (file_exists($path)) {
    extract($arr);
...
?>
      ...
      <ul style="margin-left:10px;" style="display:none" name="ba0" id="ba0">

      <?php
    } else {
      ?>

        <ul style="margin-left:10px;" class="hide" name="ba<?php echo $ide['id']; ?>" id="ba<?php echo $ide['id']; ?>">
      ...
      include $path;
        ?> </ul> <?php
              ...
              function getPosts()
              {
                ...
                $conn = db_connect();
                try {
                  $post = $conn->query("SELECT * FROM `posts`")->fetchAll(PDO::FETCH_ASSOC);
                } catch (PDOException $e) {
                  print "Ошибка!: " . $e->getMessage();
                  die();
                }
